...
class Fin_morphism: 
    ...
    alpha.map          = the tuple of length m whose ith entry is = 0          if alpha(i) = *, and 
                                                                  = alpha(i)   otherwise.
    """
    def __init__(self, domain: int, codomain: int, map_: list, name: str = ''):
        self.domain     = domain
    ...
    def compose(self, beta):
        ...
        for value in self.map:
            if value>0:
                ...
                composite.append(0)
        result = Fin_morphism(self.domain,beta.codomain,composite)
        return result
    ...
    def sum(self, beta):
        ...
        return Fin_morphism(self.domain+beta.domain, self.codomain+beta.codomain, self.map+shifted)
...
class Tuple_morphism:
    ...
    """
    def __init__(self, domain: list, codomain: list, map: list, name: str = ''):
        self.domain         = domain
        self.codomain       = codomain
        self.map            = map
        self.name           = name
    ...
    def compose(self, g: 'Tuple_morphism') -> 'Tuple_morphism':
        ...
        g_o_f = Tuple_morphism(self.domain,g.codomain,self.underlying_map.compose(g.underlying_map).map)
        return g_o_f
