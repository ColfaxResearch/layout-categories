...
class SpanMorphismCompositionTest(Scene):
    ...
    def _play_example(self, example: SpanCompositionExample) -> None:
        ...
        # leg.  f₁'s arrows give up their carets and fan out into the blocks;
        # the strands to Y become parallel, cell for cell.
        self.play(
        ...
        # middle is becoming a second copy of Y, cell for cell.
        self.remove(
            *v_cells,
            *second_fans.values(),
        ...
        # --- Stage 2: reorder the blocks into the order of X'.  A V cell -----
        # outside f's image has no place in X', so its block leaves.  The
        # morphism data migrates in place: f's arrows have become b′'s fans
        # and b's fans have become f′'s arrows, so each label transforms
        # where it stands.
