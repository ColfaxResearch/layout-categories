...
scene runs those steps in place on five evenly spaced stacks:
    ...
    pushforward: [S] -f-> [X] -g'-> [Y'] -a'-fans-> [Y] -b-fans-> [U]
...
class CoSpanMorphismCompositionSingleBeatTest(Scene):
    ...
    def _play_example(self, example: CoSpanCompositionExample) -> None:
        ...
        # --- The pushforward in one beat: every middle cell splits and its ---
        # pieces travel straight to their Y' slots, so the split and the
        # reordering are one motion.  g's arrows give up their carets and
        # fan out toward Y as they go, the strands to X cross into g',
        # a T cell sent to the basepoint by g fades with its block unsplit,
        # and the arrivals join the same beat.
        self.play(
        ...
        # blocks with no place in Y' keep their originals, to fade unsplit.
        self.remove(
            *(
                t_cells[mode]
